...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (file_exists($_FILES['product_image']['tmp_name']) || is_uploaded_file($_FILES['product_image']['tmp_name'])) {
        ...
        if (move_uploaded_file($_FILES['product_image']['tmp_name'], "../" . $pathImage)) {
            $sql = "INSERT INTO product(admin_id, category_id, brand_id, product_name, product_detail, product_price, product_qty, product_image, product_use) VALUE($admin_id, $category_id, $brand_id, '$product_name', '$product_detail', $product_price, $product_qty, '$pathImage', $product_use)";
        } else {
            echo "<script>
                Swal.fire({
