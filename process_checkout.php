        ...
        try {
          ...
          $stmt->bind_param("sssssssss", $order_fullname, $order_address, $order_tel, $order_bank, $order_amount, $datatimeorder, $order_slip, $product_name, $order_status);
            ...
            if (!empty($items) && is_array($items)) {
              ...
              $sql_order_detail = "INSERT INTO order_detail(order_id, product_id, product_name, product_price, order_qty, order_subtotal) VALUES (?, ?, ?, ?, ?, ?)";
              $stmt_order_detail = $conn->prepare($sql_order_detail);
              $stmt_order_detail->bind_param("iissdd", $order_id, $product_id, $product_name, $product_price, $order_qty, $order_subtotal);
              $stmt_order_detail->execute();
        
              // Update product quantity in the product table
              $sql_update_product = "UPDATE product SET product_qty = product_qty - ? WHERE product_id = ?";
              $stmt_update_product = $conn->prepare($sql_update_product);
              $stmt_update_product->bind_param("ii", $order_qty, $product_id);
