  ...
  if($_SERVER['REQUEST_METHOD'] === 'POST'){
    try {
      ...
      if (isset($_FILES['slip']) && $_FILES['slip']['error'] === UPLOAD_ERR_OK) {
        ...
        $slipExtension = pathinfo($slipFileName, PATHINFO_EXTENSION);

        if (in_array($slipExtension, $allowedExtensions)) {
          $slipUploadDirectory = "../slip";
          $ref = substr(md5(rand() . time()), 0, 8);
          $slipPath = $slipUploadDirectory . $ref . "." . $slipExtension;

          if (move_uploaded_file($slipTempName, $slipPath)) {
